...
func SocketXDPGetInfo(ino uint32, cookie uint64) (*XDPDiagInfoResp, error) {
	...
	req.AddData(&xdpSocketRequest{
		...
		Ino:    ino,
		Cookie: [2]uint32{uint32(cookie), uint32(cookie >> 32)},
	})
	...
	if msgs[0].Header.Type == unix.NLMSG_ERROR {
		error := int32(native.Uint32(msgs[0].Data[0:4]))
		return nil, syscall.Errno(-error)
	}

	sockInfo := &XDPSocket{}
	if err := sockInfo.deserialize(msgs[0].Data); err != nil {
		return nil, err
	}
	attrs, err := nl.ParseRouteAttr(msgs[0].Data[sizeofXDPSocket:])
	if err != nil {
		return nil, err
	}
	return attrsToXDPDiagInfoResp(attrs, sockInfo)
}
